document.addEventListener("DOMContentLoaded", () => {
    ...
    try {
      ...
      const participantsHTML = hasParticipants
        ? `<ul class="participants-list">${details.participants
            .map(
              (p) =>
                `<li class="participant-item" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(
                  p
                )}">${p}<button class="participant-remove" aria-label="Remove ${p}">✕</button></li>`
            )
            .join("")}</ul>`
  ...
  activitiesList.addEventListener("click", async (event) => {
    ...
    try {
      ...
      messageDiv.textContent = "Unregister request failed. Please try again.";
      messageDiv.className = "error";
      messageDiv.classList.remove("hidden");
